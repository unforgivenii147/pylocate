...
def updatedb():
    ...
    parser = argparse.ArgumentParser(description='Update PyLocate database', prog='updatedb-py')
    parser.add_argument('paths', nargs='*', help='Paths to index (default: home and system paths)')
...
def main():
    ...
    parser.add_argument('pattern', nargs='?', help='Search pattern (supports * and ? wildcards)')
    parser.add_argument('--database', '-d', help='Database file path')
    parser.add_argument(
        '--limit', '-l', type=int, default=1000, help='Limit number of results (default: 1000)'
    )
    parser.add_argument('--count', '-c', action='store_true', help='Only show count of matches')
    parser.add_argument('--ignore-case', '-i', action='store_true', help='Ignore case distinctions')
    parser.add_argument('--basename', '-b', action='store_true', help='Match only the base name')
